...
for i=1:length(rank_info)
    for j=1:m
        [temp index_sort]=sort(pop(index+1:index+rank_info(i),n+j));%排序(同一等级的排序计算距离)
        index_sort=index_sort+index;%记录排序位置
        for k=1:rank_info(i)
            newpos=find(index_sort==index+k);%找到个体k排序后的位置，然后求聚焦距离
            if(newpos==1||newpos==rank_info(i))
                pop(index+k,m+n+2)=inf; % 在边界,距离为无穷大
            else
                pre_person_index=index_sort(newpos+1);
                next_person_index=index_sort(newpos-1);
                pop(index+k,m+n+2)=pop(index+k,m+n+2)+(pop(pre_person_index,n+j)-pop(next_person_index,n+j))/m;
            end
        end
    end
