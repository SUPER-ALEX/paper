...
for t = 1 : M
   ...
   if( mod( t, G ) == 1 )   % 每迭代G次之后，进行鸡群的等级划分
        ...
        motherLib = randperm( hNum, mNum ) + rNum;   % 母鸡的索引，母鸡是在雌鸡中挑选的  前rNum为公鸡。
       ...
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        mate = randpermF( rNum, hNum ); %每个公鸡代表一个组，可能包含多个母鸡
        
        % Randomly select cNum chicks' mother hens
        mother = motherLib( randi( mNum, cNum, 1 ) );   % 给小鸡选择母鸡，每个小鸡对应一个母鸡的索引
   end
    
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
   for i = 1 : rNum                % Update the rNum roosters' values.  更新公鸡的值
    ...
    for i = ( rNum + 1 ) : ( rNum + hNum )  % Update the hNum hens' values. 更新母鸡的值
    ...
    for i = ( rNum + hNum + 1 ) : pop    % Update the cNum chicks' values.  更新小鸡的值
        x( sortIndex( i ), : ) = pX( sortIndex( i ), : ) + ( pX( ...
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   % Update the individual's best fitness vlaue and the global best one  更新个体最优和全局最优
...
%  the value of "tabu"
function value = randiTabu( min, max, tabu, dim )  % 返回一个dim * 1 的数组，里面数字都是 (min, max) 之间的，且都不相等，都不等于 tabu
value = ones( dim, 1 ) .* max .* 2;
num = 1;
while ( num <= dim )
    temp = randi( [min, max], 1, 1 ); %  randi（iMax，m，n）在开区间（0，iMax）生成mXn型随机矩阵， r = randi([iMin,iMax],m,n)在开区间（iMin，iMax）生成mXn型随机矩阵
    if( length( find( value ~= temp ) ) == dim && temp ~= tabu )  % find 返回的是索引位置，返回是个数组  length( find( value ~= temp ) ) == dim 表示 value中没有一个值是等于temp的
        value( num ) = temp;
        num = num + 1;
    end
end
...
%--------------------------------------------------------------------------
function result = randpermF( range, dim )  % 返回 1*(range+dim)数组，前range个为1~range打散不重复，后dim个数为 1~range的随机数，可能重复
...
temp = randperm( range, range );  % p = randperm(n,k) 返回一行从1到n的整数中的k个，而且这k个数也是不相同的
